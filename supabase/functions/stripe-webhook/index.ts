    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (RESEND_API_KEY) {
        ...
        const resendResponse = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            from: "kCaliper <onboarding@resend.dev>", // Cambiado temporalmente porque kcaliper.com no está verificado
            to: [customerEmail],
            subject: htmlTitle,
            html: welcomeHtml,
          }),
        ...
        if (!resendResponse.ok) {
           console.error("Resend API falló:", JSON.stringify(resendData));
        } else {
